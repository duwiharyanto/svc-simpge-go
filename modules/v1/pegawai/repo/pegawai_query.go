...
func getListAllPegawaiPrivateQuery(req *model.PegawaiPrivateRequest) string {
	...
	COALESCE((SELECT (SELECT COUNT(*) from hcm_personal.personal_hubungan_keluarga phk WHERE phk.id_personal_data_pribadi = p.id_personal_data_pribadi AND phk.kd_hubungan_keluarga IN ('AAK','AT','AN') AND phk.flag_aktif = 1) + (SELECT COUNT(*) from hcm_personal.personal_hubungan_keluarga phk WHERE phk.id_personal_data_pribadi = p.id_personal_data_pribadi AND (phk.kd_hubungan_keluarga = 'SUA' OR phk.kd_hubungan_keluarga = 'IS') AND phk.flag_aktif = 1) jumlah_keluarga),'') jumlah_keluarga,
