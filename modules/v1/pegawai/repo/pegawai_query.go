...
func getListAllPegawaiPrivateQuery(req *model.PegawaiPrivateRequest) string {
	...
	COALESCE((select count(*)from (SELECT distinct phk.id_personal_data_pribadi_relasi from hcm_personal.personal_hubungan_keluarga phk WHERE phk.id_personal_data_pribadi = p.id_personal_data_pribadi AND phk.kd_hubungan_keluarga IN ('AAK','AT','AN') AND phk.flag_aktif = 1 UNION SELECT distinct phk.id_personal_data_pribadi_relasi from hcm_personal.personal_hubungan_keluarga phk WHERE phk.id_personal_data_pribadi = p.id_personal_data_pribadi AND (phk.kd_hubungan_keluarga = 'SUA' OR phk.kd_hubungan_keluarga = 'IS') AND phk.flag_aktif = 1) keluarga),'') jumlah_keluarga,
